...
def download_excel_data_without(request):
	today = "pivot {}".format(datetime.datetime.today().strftime("%d.%m.%Y"))
	...
	changed_data = pd.DataFrame(get_update_data_not_fix()[1].values('name_col', 'id_kpi'))
	...
	for my_row in get_update_data_not_fix()[0]:
		...
		if changed_data[(changed_data.name_col=='method') & (changed_data.id_kpi==my_row.id)].shape[0]!=0:
			ws.write(row_num, 8, swap_dict_method[my_row.method], format)
		else:
			ws.write(row_num, 8, swap_dict_method[my_row.method])
